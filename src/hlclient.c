...
int tty_show_user_changes = 1;
int tty_show_user_joins = 1;
int tty_show_user_parts = 1;
int tty_chat_pretty = 1;
int g_strip_ansi = 0;
int chat_colorz = 0;
int allow_foreign_hosts = 0;
int remote_client = 0;
int remote_host_port = HTLS_TCPPORT;
int default_secure = 1;
int do_readme = 0;
int pid_username = 0;
int pid_username_force = 0;

static void
set_str (char **infpre, const char *str)
{
   *infpre = xstrdup(str);
}

void
set_int (int *intp, const char *str)
{
   *intp = atoi(str);
}
...
init_vars (void)
{
   ...
   variable_add(&g_info_prefix, set_str, "infoprefix");
   g_remote_host = xstrdup(DEFAULT_HOST);
   variable_add(&g_remote_host, set_str, "remote_host");
   g_readme = xstrdup(DEFAULT_README);
   variable_add(&g_readme, set_str, "readme");
   g_autoload = xstrdup(DEFAULT_AUTOLOAD);
   variable_add(&g_autoload, set_str, "autoload");
   ...
   variable_add(&tty_show_user_changes, set_bool, "tty_show_user_changes");
   ...
   variable_add(&remote_host_port, set_int, "remote_host_port");
   variable_add(&default_secure, set_bool, "default_secure");
   variable_add(&allow_foreign_hosts, set_bool, "allow_foreign_hosts");
   variable_add(&remote_client, set_bool, "remote_client");
   variable_add(&pid_username, set_bool, "pid_username");
   variable_add(&pid_username_force, set_bool, "pid_username_force");
}
...
nick_complete (struct htlc_conn *htlc, char *buffer)
{
   ...
   if (!best_user)
      return 0;
   memcpy(matching_nick, best_user->name, maxlen);
...
hotline_client_input (struct htlc_conn *htlc, struct hx_chat *chat, char *str)
{
   if (*str) {
      if (*str == '/' && *++str && *str != '/') {
         ...
         if (chat)
            hx_send_chat(htlc, chat->cid, str);
         else
...
hotline_client_init (int argc, char **argv)
{
   ...
   opt.ind = 0;
   while ((i = getopt_r(argc, argv, "ql:", &opt)) != EOF) {
      switch (i) {
         case 'l':
            load_this = opt.arg;
            break;
   ...
   strncpy(hx_homedir, home ? home : ".", sizeof(hx_homedir) - 1);
   hx_homedir[MAXPATHLEN-1] = 0;
   strncpy(hx_user, user ? user : ".", sizeof(hx_user) - 1);
   ...
   if (pid_username)
   {
      ...
      len = strlen(hx_user);
      pidlen = snprintf(&pidstr, sizeof(pidstr)-1, "%i", getpid());
      if ( pidlen > 0 )
      {
         pidstr[pidlen] = 0;
         if ((len + pidlen) > 31)
         ...
         hx_user[len] = 0;
         strcat(&hx_user[len], pidstr);
      }
      strncpy(hx_htlc.name, hx_user, 31);
   }
   else
   {
      if (user)
   ...
   if (g_readme && strlen(g_readme)) {
      do_readme = 1;
      rdmebuf = (char *)malloc(PATH_MAX + 10);
      snprintf(rdmebuf, PATH_MAX + 10, "exec cat %s", g_readme);
      hx_command(&hx_htlc, tty_chat_front, rdmebuf);
      xfree(rdmebuf);
   }
   if (g_autoload && strlen(g_autoload)) {
      char path[MAXPATHLEN];

      expand_tilde(path, g_autoload);
      if (!access(path, R_OK))
         hx_load(&hx_htlc, 0, path);
   }
}
